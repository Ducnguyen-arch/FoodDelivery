import { useContext } from "react"
import "./Cart.css"
import { StoreContext } from "../../context/StoreContext"
import { Link, useNavigate } from "react-router-dom"
import {calculateCartTotal, formatCurrencyUnit} from "../../util/CartUtils"



const Cart = () => {

    const formatPrice = formatCurrencyUnit

    const navigate = useNavigate()
    const {foodList, increaseQuantity, decreaseQuantity, quantities, removeFromCart} = useContext(StoreContext)

    //cart items
    const cartItems = foodList.filter(food => quantities[food.id] > 0)

   const {subTotal, shipping, tax, total} = calculateCartTotal(
      cartItems,
      quantities
   )

  return (
    <div className="container py-5">
    <h1 className="mb-5">Giỏ hàng của bạn</h1>
    <div className="row">
        <div className="col-lg-8">
        {
            cartItems.length === 0 ? (
                <p>Giỏ hàng trống.</p>
            ) : (
            <div className="card mb-4">
                <div className="card-body">
                  {cartItems.map((food) => (
                    <div key={food.id} className="row cart-item mb-3">
                        <div className="col-md-3">
                            <img src={food.imageUrl} alt={food.name} className="img-fluid rounded" width={100}/>
                        </div>
                        <div className="col-md-5">
                            <h5 className="card-title">{food.name}</h5>
                            <p className="text-muted">{food.category}</p>
                        </div>
                        <div className="col-md-2">
                            <div className="input-group">
                                <button className="btn btn-outline-secondary btn-sm" type="button" onClick={() => decreaseQuantity(food.id)}>-</button>
                                <input style={{"maxWidth":"100px"}} type="text" className="form-control  form-control-sm text-center quantity-input" value={quantities[food.id]} readOnly/>
                                <button className="btn btn-outline-secondary btn-sm" type="button" onClick={() => increaseQuantity(food.id)}>+</button>
                            </div>
                        </div>
                        <div className="col-md-2 text-end">
                            <p className="fw-bold">{formatPrice(food.price * quantities[food.id])} VND</p>
                            <button className="btn btn-sm btn-outline-danger" onClick={() => removeFromCart(food.id)}>
                                    <i className="bi bi-trash"></i>
                                </button>
                        </div>
                        <hr/>
                    </div>
                  ))}
                </div>
            </div>
            )
        }

            <div className="text-start mb-4">
                <Link to="/" className="btn btn-outline-primary">
                    <i className="bi bi-arrow-left me-2"></i>Tiếp tục mua
                </Link>
            </div>
        </div>
        <div className="col-lg-4">
            <div className="card cart-summary">
                <div className="card-body">
                    <h5 className="card-title mb-4">Chi tiết thanh toán</h5>
                    <div className="d-flex justify-content-between mb-3">
                        <span>Tổng tiền món ăn</span>
                        <span>{formatPrice(subTotal)} VND</span>
                    </div>
                    <div className="d-flex justify-content-between mb-3">
                        <span>Chi phí vận chuyển</span>
                        <span>{formatPrice(subTotal === 0 ?  0.0 : shipping)} VND</span>
                    </div>
                    <div className="d-flex justify-content-between mb-3">
                        <span>Thuế <i>(5%)</i></span>
                        <span>{formatPrice(tax)} VND</span>
                    </div>
                    <hr/>
                    <div className="d-flex justify-content-between mb-4">
                        <strong>Tổng thanh toán</strong>
                        <strong>{formatPrice(subTotal === 0 ? 0.0 : total)} VND</strong>
                    </div>
                    <button className="btn btn-primary w-100" disabled={cartItems.length === 0} onClick={() => navigate('/order')}>Thanh toán</button>
                </div>
            </div>
        </div>
    </div>
</div>

  )
}

export default Cart